### BMIQ.R & CheckBMIQ.R
### fix bug of sample function. refer to NORMALIZATION.R of Horvath.
### append parameter dir_plot, i.e. directory for plot
...
BMIQ <- function(beta.v,design.v,nL=3,doH=TRUE,nfit=10000,th1.v=c(0.2,0.75),th2.v=NULL,niter=5,tol=0.001,plots=TRUE,dir_plot = ".",sampleID=1){
...
# bug. rand.idx <- sample(1:length(beta1.v),nfit,replace=FALSE)
# fix from NORMALIZATION.R from Horvath
set.seed(1)
...
pdf(paste0(dir_plot,"/Type1fit-",sampleID,".pdf"),width=6,height=4);
...
pdf(paste0(dir_plot,"/Type2fit-",sampleID,".pdf"),width=6,height=4);
...
if(plots){
 ...
 pdf(paste0(dir_plot,"/CheckBMIQ-",sampleID,".pdf"),width=6,height=4)
